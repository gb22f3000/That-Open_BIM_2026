/**
 * GB Creation Digital Twin Authentication Service
...
class AuthService {
  constructor() {
    ...
    this.users = {
      ...
      'control@gbcreation.in': {
        id: 'usr_002',
        username: 'control@gbcreation.in',
        password: 'control123',
        name: 'Rajesh Kumar',
      ...
      'maintenance@gbcreation.in': {
        id: 'usr_003',
        username: 'maintenance@gbcreation.in',
        password: 'maint123',
        name: 'Amit Sharma',
      ...
      'safety@gbcreation.in': {
        id: 'usr_004',
        username: 'safety@gbcreation.in',
        password: 'safety123',
        name: 'Priya Singh',
      ...
      'viewer@gbcreation.in': {
        id: 'usr_005',
        username: 'viewer@gbcreation.in',
        password: 'viewer123',
        name: 'Guest User',
  ...
  loadExistingSession() {
    ...
    const savedSession = localStorage.getItem('gb_creation_session');
    if (savedSession) {
      try {
  ...
  async login(username, password) {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        // Save session
        localStorage.setItem('gb_creation_session', JSON.stringify(sessionData));
  ...
  logout() {
    ...
    localStorage.removeItem('gb_creation_session');
  ...
  startSessionTimer() {
    ...
    const session = JSON.parse(localStorage.getItem('gb_creation_session'));
    if (session) {
  ...
  logAuditEvent(action, description) {
    ...
    const existingLogs = JSON.parse(localStorage.getItem('gb_creation_audit_logs') || '[]');
    existingLogs.push(auditLog);
    localStorage.setItem('gb_creation_audit_logs', JSON.stringify(existingLogs));
  ...
  getAuditLogs() {
    ...
    return JSON.parse(localStorage.getItem('gb_creation_audit_logs') || '[]');
  }
